    ...
    io.on('connection', (socket) => {
        ...
        socket.on('create-agent', async (settings, callback) => {
            ...
            if (settings.profile?.name) {
                const existing = agent_connections[settings.profile.name];
                if (existing) {
                    // Block only if the agent is genuinely in the world. A lingering
                    // socket without in_game means the previous run failed to spawn
                    // (e.g. the world was paused / LAN not ready) and left a zombie
                    // process — evict it so the user can re-enter without restarting.
                    if (existing.in_game) {
                        callback({ success: false, error: 'Agent already exists' });
                        return;
                    }
                    console.log(`Evicting stale agent record: ${settings.profile.name}`);
                    mindcraft.destroyAgent(settings.profile.name);
                    delete agent_connections[settings.profile.name];
                }
        ...
        // List character profiles found in profiles/characters/ so the UI can offer a
        // dropdown to pick a character and send it into the world with one click.
        socket.on('list-characters', (callback) => {
            try {
                const charDir = path.join(__dirname, '../../profiles/characters');
                const files = readdirSync(charDir).filter(f => f.endsWith('.json'));
                const characters = [];
                for (const file of files) {
                    try {
                        const profile = JSON.parse(readFileSync(path.join(charDir, file), 'utf8'));
                        characters.push({ file, name: profile.name || file.replace(/\.json$/, ''), profile });
                    } catch (e) {
                        console.warn(`Skipping invalid character profile ${file}: ${e.message}`);
                    }
                }
                // connection defaults pulled from settings.js so the UI doesn't need them
                const defaults = {
                ...
                callback({ success: true, characters, defaults });
            } catch (e) {
