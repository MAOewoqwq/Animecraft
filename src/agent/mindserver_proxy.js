...
class MindServerProxy {
    ...
    getNumOtherAgents() {
        // only count agents that are actually in the world (excluding self), so a
        // single online bot still answers open chat even if stale/offline agents
        // remain in the registry.
        return this.agents.filter(a =>
            a.name !== this.agent?.name && (a.in_game || a.socket_connected)
        ).length;
    }
