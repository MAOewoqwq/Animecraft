...
const modes_list = [
    ...
    {
        name: 'easter_eggs',
        description: 'Data-driven character easter eggs: say a short in-character line when a configured entity is spotted. Eggs are defined per-character in the profile\'s "easter_eggs" array. Does not interrupt actions.',
        interrupts: ['all'], // run every tick (even mid-action) so it can comment any time; it never calls execute, so nothing is actually interrupted
        on: true,
        active: false,
        last_fired: {}, // per-egg cooldown timestamps, keyed by egg index
        update: function (agent) {
            const eggs = agent.prompter.profile.easter_eggs;
            if (!Array.isArray(eggs) || eggs.length === 0) return;
            const now = Date.now();
            for (let i = 0; i < eggs.length; i++) {
                const egg = eggs[i];
                if (!egg || !Array.isArray(egg.lines) || egg.lines.length === 0) continue;
                const cooldown = (egg.cooldown ?? 60) * 1000;
                if (now - (this.last_fired[i] || 0) < cooldown) continue;
                const range = egg.range ?? 12;
                // trigger can be the special keyword 'hostile' (any hostile mob) or an entity name
                const predicate = egg.trigger === 'hostile'
                    ? (entity => mc.isHostile(entity))
                    : (entity => entity.name === egg.trigger);
                const target = world.getNearestEntityWhere(agent.bot, predicate, range);
                if (target) {
                    const line = egg.lines[Math.floor(Math.random() * egg.lines.length)];
                    say(agent, line);
                    this.last_fired[i] = now;
                }
            }
        }
    },
